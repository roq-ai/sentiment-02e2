...
import { getBrokerById } from 'apiSdk/brokers';
import { BrokerInterface } from 'interfaces/broker';
import { ForexPairListPage } from 'pages/forex-pairs';

function BrokerViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BrokerInterface>(
    () => (id ? `/brokers/${id}` : null),
    () =>
      getBrokerById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('broker', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/brokers/edit/${id}`} passHref legacyBehavior>
                    <Button
