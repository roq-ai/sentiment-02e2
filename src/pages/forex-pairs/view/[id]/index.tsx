...
import { getForexPairById } from 'apiSdk/forex-pairs';
import { ForexPairInterface } from 'interfaces/forex-pair';
import { TradingDataListPage } from 'pages/trading-data';

function ForexPairViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('forex_pair', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/forex-pairs/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Pair Name" text={data?.pair_name} />
                ...
                {hasAccess('broker', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Broker"
                    text={
                      <Link as={NextLink} href={`/brokers/view/${data?.broker?.id}`}>
                        {data?.broker?.name}
                      </Link>
                    }
                  />
